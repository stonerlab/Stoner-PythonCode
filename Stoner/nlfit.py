...
if python_v3:
    ...
    import ConfigParser
import pylab
pylab.ion()  #interactive mode, works best with iPython
...
class NLFit:
    ...
    def __init__(self, ini_file, func, data=None, chi2mapping=False):
        ...
        for section in parnames:
            pars = {
                'value': self.config.getfloat(section, 'value'),
                'fixed': self.config.getboolean(section, 'fixed'),
                'limited': (self.config.getboolean(section, 'lower_limited'), self.config.getboolean(section,
                                                                                                     'upper_limited')),
...
#run sim
    ...
    def _runfit(self):
        ...
        if self.data_input == None:
            ...
            assert (isinstance(self.data_input, Stoner.AnalyseFile) or isinstance(self.data_input, Stoner.DataFile))
