...
def peaks(datafile, **kargs):
    ...
    Keyword Arguments:
        ...
        modify (book):
            If true, then the returned object is a copy of datafile with only the peaks/troughs left in the data.
        full_data (bool):
    ...
    setas = datafile.setas.clone  # pylint: disable=E0203
    datafile.setas = ""
    d1 = datafile.SG_Filter(ycol, xcol=xcol, points=width, poly=poly, order=1).ravel()
    d2 = datafile.SG_Filter(
    ...
    d2_interp = sp_interp1d(np.arange(len(d2)), d2, kind="cubic")
    ...
    if modify:
        datafile.data = datafile.interpolate(xdat, xcol=xcol, kind="cubic")
        ret = datafile
    elif full_data:
        ret = datafile.interpolate(xdat, kind="cubic", xcol=False)
    else:
        ret = xdat
...
def find_peaks(datafile, **kargs):
    ...
    Keyword Arguments:
        ...
        modify (book):
            If true, then the returned object is a copy of datafile with only the peaks left in the data.
        full_data (bool):
            If True (default) then all columns of the data at which peaks in the *ycol* column are found.
            *modify* true implies *full_data* is also true. If *full_data* is False, then only the x-column
            values of the peaks are returned.
    ...
    if sort and sort_key:
        ...
        for k in data:
            data[k] = data[k][idx]
    xmin, xmax = datafile.span(_.xcol)
    xconv = len(datafile) / (xmax - xmin)
    ...
    for k, v in data.items():
        if prefix is None:
            ...
            datafile[f"{prefix}:{k}"] = v
    if modify:
        datafile.data = peak_data
        return datafile
    if full_data:
