...
def gridimage(im, points=None, xi=None, method="linear", fill_value=None, rescale=False):
    ...
    Args:
        im (ImargeArray,ImageFile):
            Image data to be worked with.
        points (tuple of (x coordinates, y coordinates)):
            The actual sampled data coordinates.
        xi (tuple of (2D array, 2D array)):
            The regular grid coordinates (as generated by, for example, :py:func:`numpy.meshgrid`).

    Keyword Arguments:
        method ("linear","cubic","nearest"):
            how to interpolate, default is linear
        fill_value (float, Callable, None):
    ...
    Returns:
        A copy of the modified image. The image data is interpolated and the metadata keys ``actual_x``,
        ``actual_y``, ``sample_x`` and ``sample_y`` give the coordinates of the new grid.
    ...
    Notes:
        ...
        used. These are set, for example, by the STXM image loader if the interferometer stage
...
def crop(image, *args, **kwargs):
    ...
    Notes:
        ...
        The box can be an integer border, a fractional border between 0 and 1,
        a metadata-key string, or a four-item tuple. Tuple entries may be pixel
        coordinates, fractional offsets from the corresponding edge, or ``None``
        to use the full extent on that edge.
