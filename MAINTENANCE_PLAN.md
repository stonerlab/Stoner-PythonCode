...
- Review the remaining `TODO`/`FIXME` cases, including the metadata-copy workaround and incomplete column-indexing behaviour, with regression evidence before changing behaviour.
  ...
  Column-bounds review confirmed original extrema indices and identified temporary-mask restoration defects; see `maintenance/phase7/column-bounds.md` for the focused fix and validation.
- Fixed the recorded same-dtype `ImageStack.convert(force_copy=True)` failure with a direct NumPy copy, preserving masks and independent pixel storage. Evidence: `maintenance/phase7/image-convert-copy.md`.
...
- Audited dynamic Data method binding: all 85 methods preserve signatures, documentation and annotations, appear in dir/help, and match the Sphinx inventory on normal/RTD Python 3.14 and normal Python 3.11. No binding fix is indicated. Source annotation gaps, the threshold return annotation, runtime typing placeholders and decorator documentation need a separate decision; see `maintenance/phase7/dynamic-binding-audit.md`.
  Approved follow-up completed: corrected threshold annotations, replaced placeholder typing classes with resolvable deferred references, fixed the Image import case and class_modifier documentation. Focused checks pass on Python 3.11/3.14; all 458 tests collect and the 85-method audit remains clean. Evidence: `maintenance/phase7/typing-fixes.md`.
...
Compatibility helpers and deprecated APIs still have callers; removing obsolete
branches or consolidating duplicated helpers requires a separately justified,
...
- Correcting code to match its documented contract is bug fixing. Document runtime corrections carefully; backward-compatible API additions are acceptable in minor point releases, while API-breaking changes require a larger release.
...
### Phase 6 Data and plotting docstrings complete (2026-09-12)

Corrected the Data overview, Setas.__call__ and plot_xyuv documentation. Only
docstrings changed in package source (AST verified); focused probes confirm
Setas identity, role-list and reset behaviour. Clean RTD-mode warnings fall from
470 to 443. All five primary classes and 85 dynamic Data methods remain documented.
Only the obsolete Data.patterns and Data.subclasses inventory entries were removed,
with explicit review and runtime absence evidence; no unexpected entries were lost.
All 243 plot-cache hashes are unchanged. Evidence: `maintenance/phase6/batch4.md`.
