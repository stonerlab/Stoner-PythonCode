...
As one might expect form the name, the \textit{subtract} method subtracts the second column form the first. Like \textit{normalise} the first data column with be replaced and a new header (defaulting to column header 1 - column header 2) will be created, but can be overridden with the header and replace keyword arguments. The next two variants of the \textit{subtract} method work in an analogous manner to the \textit{normalise} methods. Finally the \text{add} method allows one to add two columns in a similar fashion:
