...
The :py:meth:`Stoner.Data.normalise` method simply divides the data column by the reference column. By default the normalise method
...
As one might expect form the name, the :py:meth:`Stoner.Data.subtract` method subtracts the second column form the first.
Unlike :py:meth:`Stoner.Data.normalise` the first data column will not be replaced but a new column inserted and a new header
(defaulting to 'column header 1 - column header 2') will be created. This can be overridden with the *header* and *replace*
keyword arguments. The next two variants of the :py:meth:`Stoner.Data.subtract` method work in an analogous manner to the
:py:meth:`Stoner.Data.normalise` methods. Finally the :py:meth:`Stoner.Data.add` method allows one to add two columns in a
similar fashion::
...
The final variant for these channel operations is the :py:meth:`Stoner.Data.diffsum` which takes the ratio of the difference over the sum of two channels.
...
:py:meth:`Stoner.Data.diffsum` method where the error propagation is not entirely trivial.
...
each row belongs in. The :py:meth:`Stoner.Data.split` method is useful for this case.::
...
:py:meth:`Stoner.Data.max` and :py:meth:`Stoner.Data.min`::
...
The :py:meth:`Stoner.Data.span` method simply returns a tuple of minimum and maximum values within either the whole column or
bounded data. Internally this is just calling the :py:meth:`Stoner.Data.max` and :py:meth:`Stoner.Data.min` methods.
The :py:meth:`Stoner.Data.clip` method deletes rows for which the specified column as a value that is either larger or
smaller than the maximum or minimum value within the second argument. This allows one to specify either a tuple --
eg the result of the :py:meth:`Stoner.Data.span` method, or a complete list as in the last example above. Specifying a single
...
:py:class:`Stoner.Data` offers a number of methods to assist in data reduction and data processing.
...
:py:class:`Stoner.Data` provides a simple :py:meth:`Stoner.Data.bin` method that can re-bin data::
...
points averaged into each bin.. If *clone* is True or not provided, :py:meth:`Stoner.Data.bin` returns a clone of the current data file with its data
...
underlying trends. The Stoner package offers a  number of approaches to filtering data.
    ...
    - Smoothing by convoluting with a window
        ...
        defined by the window function. This is handled by the :py:meth:`Stoner.Data.smooth` method.::
        ...
        is **False** in which case, the smoothed data is passed back as the return value and the current :py:class:`Stoner.Data`
        is left unmodified.
    ...
    - Savitzky-Golay filtering
        ...
        :py:meth:`Stoner.Data.SG_Filter` method.
    ...
    - Spline
        ...
        implemented as the :py:meth:`Stoner.Data.spline` method.
...
best mapping between two sets of (x,y) points that are nominally the same. :py:class:`Stoner.Data` provides
a :py:meth:`Stoner.Data.stitch` method to facilitate this.
...
In addition to changing the X and Y data in the current :py:class:`Stoner.Data`
...
:math:`y=f(x)-y_{threshold}`). This is carried out by the :py:meth:`Stoner.Data.threshold` method::
...
Interpolating data finds values of y for points x that lie between data points. The :py:meth:`Stoner.Data.interpolate` provides a simple pass-through to the
...
The :py:meth:`Stoner.Data.interpolate` method will return values that are obtained from 'joining the dots' - which is
appropriate if the uncertainties (and hence scatter) in the data is small. With more scatter in the data, it is better to
use some locally fitted spline function to interpolate with. The :py:meth:`Stoner.Data.spline` function can be used for this.::
...
:py:meth:`Stoner.Data.spline` method returns a :py:class:`scipy.interpolate.UnivararateSpline` object that can be used to
...
the :py:class:`Stoner.Data` instance, optionally replacing an existing column indexed by *result* and given a new header
...
The :py:meth:`Stoner.Data.peaks` provides a relatively simple and effective method for doing this::
...
The example below shows how to use :py:meth:`Stoner.Data.peaks` to filter out just the peak positions in a set of data.
