...
-   `Simple polynomial fits`_
    ...
    is the simplest and easiest to use. This makes use of the :py:meth:`~Stoner.core.data.Data.polyfit` method.
...
-   `Simple function fitting`_
    ...
    the standard :py:func:`scipy.optimize.curve_fit` function in the form of the :py:meth:`~Stoner.core.data.Data.curve_fit` method.
...
-   `Fitting with limits`_
    ...
    the you probably want to use the :py:meth:`~Stoner.core.data.Data.lmfit` method. This works well when your data has uncertainties in the *y* values but
    not in *x*.

-   `Orthogonal distance regression`_

    Finally, if your data has uncertainties in both *x* and *y* you may want to use the :py:meth:`~Stoner.core.data.Data.odr` method to do an analysis that
...
standalone fitting functions:
    ...
    #.  They provide a consistent way of defining the model to be fitted. All of the Stoner package functions accept a model function of the form:
        ``f(x, p1, p2, p3)``, constructing the necessary intermediate model class as required. Similarly, they can all take an :py:class:`lmfit.model.Model`
    ...
    #.  They provide a consistent parameter order and keyword argument names as far as possible within the limits of the underlying algorithms.
        Generally these follow the :py:func:`scipy.optimize.curve_fit` conventions.
    #.  They make use of the :py:attr:`~Stoner.core.data.Data.setas` attribute to identify data columns containing *x*, *y* and associated uncertainties. They
        also provide a common way to select a subset of data to use for fitting through the *bounds* keyword argument.
    #.  They provide a consistent way to add the best-fit data as one or more columns to the :py:class:`~Stoner.core.data.Data` object and to store the best-fit
        parameters in the metadata for later retrieval. The package can therefore use
        :py:meth:`~Stoner.core.data.Data.annotate_fit` to display the fitting parameters on a plot of the data.

Simple polynomial Fits
======================

Simple least squares fitting of polynomial functions is handled by the
:py:meth:`~Stoner.core.data.Data.polyfit` method::
...
:py:meth:`~Stoner.core.data.Data.search` to restrict the fitting to a limited range of rows. The
...
The output of the three methods used to fit arbitrary functions depends on the keyword parameters *output*, *result*, *replace* and *header* in the method call.
    ...
    -   *output="data"*
        A copy for the data file itself is returned - this is most useful when used in conjunction with :py:class:`~Stoner.folders.mixins.DataFolder`
    -   *oputput="full"*
...
is calculated and added to the metadata of the :py:class:`~Stoner.core.data.Data` object. To distinguish between multiple fits, a *prefix* keyword can be given, otherwise
...
to the :py:class:`~Stoner.core.data.Data` obhject. If *replace* is **True** then it is added to the end of the :py:class:`~Stoner.core.data.Data` object and *replace* is ignored. Otherwise,
...
The utility method of :py:class:`Stoner.core.data.Data`, :py:meth:`Stoner.core.data.Data.annotate_fit`, is useful
...
For more general curve fitting operations the :py:meth:`~Stoner.core.data.Data.curve_fit`
...
*xcol* and *ycol* are the x and y columns to fit. If *xcol* and *ycol* are not given, then the :py:attr:`~Stoner.core.data.Data.setas` attrobite is used to determine
...
identified int he :py:attr:`~Stoner.core.data.Data.setas` attribute as containing *e* values, then that is used instead.
...
The `lmfit package <https://lmfit.github.io/lmfit-py/>`_ provides a flexible, object-oriented way to fit complex models to experimental data.
A full description is given in the `lmfit documentation <https://lmfit.github.io/lmfit-py/>`_. The
:py:meth:`~Stoner.core.data.Data.lmfit` method is used to interact with lmfit.

In order to use :py:meth:`~Stoner.core.data.Data.lmfit`, one requires a :py:class:`lmfit.model.Model` instance. This describes a function
and its independent and fittable parameters, whether they have limits and what the limits are. The :py:mod:`Stoner.analysis.fitting` package contains
a series of :py:class:`lmfit.model.Model` subclasses that represent various models used in condensed matter physics.

The operation of :py:meth:`~Stoner.core.data.Data.lmfit` is very similar to that of :py:meth:`~Stoner.core.data.Data.curve_fit`::

    from Stoner.analysis.fitting.models.thermal import Arrhenius
    model=Arrhenius(A=1E7, DE=0.01)
    fit=a.lmfit(model,xcol="Temp",ycol="Cond",result=True,header="Fit")
    print fit.fit_report()
    print a["Arrehenius:A"],a["Arrehenius:A err"],a["chi^2"],a["nfev"]

In this example we fit an Arrhenius model to data contained in the 'Temp' and 'Cond' columns. The resulting
fit would be added as an additional column called fit. In addition, details of the fit are added as metadata to the current :py:class:`~Stoner.core.data.Data`.

The *model* argument to :py:meth:`~Stoner.core.data.Data.lmfit` can be either an instance of the model class, or just the class itself (in which case it will be
...
a simple call to :py:meth:`~Stoner.core.data.Data.curve_fit`.

The return value from :py:meth:`~Stoner.core.data.Data.lmfit` is controlled by the *output* keyword parameter. By default it is an :py:class:`lmfit.model.ModelResult`
...
:py:meth:`~Stoner.core.data.Data.curve_fit` and :py:meth:`~Stoner.core.data.Data.lmfit` are both essentially based on a Levenberg-Marquardt fitting algorithm which is a non-linear least squares
routine. The essential point is that it seeks to minimise the **vertical** distance between the model and the data points, taking into account the uncertainty
in the vertical position (i.e. *y* coordinate) only. If your data has peaks that may change position and/or uncertainties in the horizontal (*x*) position of
the data points, you may be better off using an orthogonal distance regression.

The :py:meth:`~Stoner.core.data.Data.odr` method wraps the :py:mod:`scipy.odr` module and follows the ``lmfit`` interface where possible. In fact, in most
...
The :py:meth:`~Stoner.core.data.Data.odr` method allows uncertainties in *x* and *y* to be specified via the *sigma_x* and *sigma_y*  parameters. If either are not specified, and
a *sigma* parameter is given, then that is used instead. If they are either explicitly set to **None** or not given, then the :py:attr:`~Stoner.core.data.Data.setas` attribute is
used instead.
...
approach may be valuable. The :py:meth:`Stoner.core.data.Data.differential_evolution` method provides a wrapper around the :py:func:`scipy.optimize.differential_evolution`
...
of the fit with the parameters. In order to provide such an estimate, the :py:meth:`Stoner.core.data.Data.differential_evolution` method carries out a standard least-squares
...
The :py:mod:`Stoner.analysis.fitting.models` module provides a number of standard fitting models suitable for solid-state physics. Each model is provided as either an
``lmfit.Model`` or a callable function. The former often also provides a means to guess initial values from the data.
...
.. currentmodule:: Stoner.analysis.fitting.models.generic
    ...
    -   :py:class:`Stoner.analysis.fitting.models.generic.Linear` - straight-line fit :math:`y=mx+c`
    -   :py:class:`Stoner.analysis.fitting.models.generic.Quadratic` - second-order polynomial :math:`y=ax^2+bx+c`
    -   :py:class:`Stoner.analysis.fitting.models.generic.PowerLaw` - a general power-law expression :math:`y=Ax^k`
    -   :py:class:`Stoner.analysis.fitting.models.generic.StretchedExp` is a standard exponential decay with an additional power :math:`\beta` -
...
.. currentmodule:: Stoner.analysis.fitting.models.thermal
    ...
    -   :py:class:`Stoner.analysis.fitting.models.thermal.Arrhenius` - The Arrhenius expression is used to describe processes whose rate is controlled by a thermal distribution,
        it is essentially an exponential decay :math:`y=A\exp\left(\frac{-\Delta E}{k_Bx}\right)`.
    -   :py:class:`Stoner.analysis.fitting.models.thermal.ModArrhenius` - The modified Arrhenius expression :math:`\tau=Ax^n\exp\left(\frac{-\Delta E}{k_B x}\right)` is used when the
    ...
    -   :py:class:`Stoner.analysis.fitting.models.thermal.NDimArrhenius` is suitable for thermally activated transport in more than one dimension, or in variable-range hopping processes -
        :math:`\tau=A\exp\left(\frac{-\Delta E}{k_B x^n}\right)`
    -   :py:class:`Stoner.analysis.fitting.models.thermal.VFTEquation` - the Vogel-Fulcher-Tammann equation is another modification of the Arrhenius equation which can apply when a process
        has both an activation energy and a threshold temperature (due to a phase transition, for example) and is given by
...
.. currentmodule:: Stoner.analysis.fitting.models.tunnelling
    ...
    -   :py:class:`Stoner.analysis.fitting.models.tunnelling.Simmons` - the Simmons model describes tunnelling through a square barrier potential for the limit where the barrier height
        is comparable to the junction bias.
    -   :py:class:`Stoner.analysis.fitting.models.tunnelling.BDR` - this model introduces a trapezoidal barrier where the barrier height is different between the two electrodes - e.g. where the
        electrodes are composed of different materials.
    -   :py:class:`Stoner.analysis.fitting.models.tunnelling.FowlerNordheim` - this is another simplified model of electron tunnelling that has single barrier-height and width parameters.
    -   :py:class:`Stoner.analysis.fitting.models.tunnelling.TersoffHammann` - this model just treats tunnelling as a linear I-V process and is applicable when the barrier height is large compared
...
-   :py:class:`Stoner.analysis.fitting.models.magnetism.Langevin` models the magnetic moment versus field of a paramagnet.
-   :py:class:`Stoner.analysis.fitting.models.magnetism.KittelEquation` and :py:class:`Stoner.analysis.fitting.models.magnetism.Inverse_Kittel` describe the magnetic field and frequency
    response of the ferromagnetic resonance peak.

Peak Models
-----------

The `lmfit <https://lmfit.github.io/lmfit-py/>`_ package comes with several common peak-function models which can be used directly. The Stoner package adds a couple more to the
selection - these are particularly useful for fitting ferromagnetic resonance data:

    -   :py:class:`Stoner.analysis.fitting.models.generic.Lorentzian_diff` - lmfit includes built-in classes for Lorentzian peaks, but this model is the differential
        of a Lorentzian peak.
    -   :py:class:`Stoner.analysis.fitting.models.magnetism.FMR_Power` - although this model is usually used specifically for calculating the absorption spectrum for a ferromagnetic resonance
...
.. currentmodule:: Stoner.analysis.fitting.models.superconductivity
    ...
    -   :py:class:`Stoner.analysis.fitting.models.superconductivity.Strijkers` - this tunnel model describes electrons passing from a superconductor to a non-superconductor with a potential step or
...
Finally, we include some other common electrical-transport models for solid-state physics in the :py:mod:`Stoner.analysis.fitting.models.e_transport` module.
.. currentmodule:: Stoner.analysis.fitting.models.e_transport

    -   :py:class:`Stoner.analysis.fitting.models.e_transport.BlochGrueneisen` - this model describes electrical resistivity of a metal as a function of temperature and can be used to
        extract the Debye temperature :math:`\Theta_D`.
    -   :py:class:`Stoner.analysis.fitting.models.e_transport.FluchsSondheimer` - this model describes the electrical resistivity of a thin film as a function of its thickness and can
        be used to extract a mean free path :math:  `\lambda_{mfp}` and intrinsic conductivity.
    -   :py:class:`Stoner.analysis.fitting.models.e_transport.WLfit` - the weak-localisation fit can be used to describe the magnetoconductance of a system with sufficient scattering that
        weak-localisation effects appear,

Making Fitting Models
=====================

You can simply pass a bare function to any of the general fitting methods, but there can be advantages in using an ``lmfit.Model`` class - such as the
...
:py:func:`Stoner.analysis.fitting.models.make_model` is a Python decorator function that can do the leg-work of transforming a simple model function of the form::
...
:py:meth:`~Stoner.core.data.Data.curve_fit` can also be used to fit more complex problems. In the example below, a set of
...
edit the source code. :py:meth:`~Stoner.core.data.Data.lmfit` and :py:meth:`~Stoner.core.data.Data.odr` combined with :py:mod:`Stoner.analysis.fitting` provide some mechanisms to enable this.
...
This initialisation file can be passed to :py:func:`Stoner.analysis.fitting.models.cfg_data_from_ini` which will use the information in the [Data]
section to read in the data file and identify the x and y columns.

The initialisation file can then be passed to :py:func:`Stoner.analysis.fitting.models.cfg_model_from_ini` which will use the configuration file to
...
returns the configured model and also a 2D array of values to feed as the starting values to :py:meth:`~Stoner.core.data.Data.lmfit`. Depending
...
Since both the :py:meth:`~Stoner.core.data.Data.odr` and :py:meth:`~Stoner.core.data.Data.differential_evolution` supports the same interface, either can be used as a
