...
The possible sub-classes are:
    ...
    :py:class:`Stoner.FileFormats.BigBlueFile` 
        Datafiles produced by VB Code running on Big Blue. The
    ...
    :py:class:`Stoner.FileFormats.XRDFile` 
        Loads a scan file produced by Arkengarthdale - the group's Brucker
        XRD Machine.
    ...
    :py:class:`Stoner.FileFormats.OpenGDAFile` Reads an ascii  scan file generated by OpenGDA -- a software 
        suite used for synchtrons such as Diamond.
...
Loading Data from a stromg or iterable object
---------------------------------------------
...
i.e. make the values visible and useable. The next example illustrates using a numpy array of booleans to 
...
the entire row -- or a list of booleans to mask individual cells in the current row.
...
and not by value. This can lead to unexcted results as you can end up modifying variables 
...
The Stoner mpodule redefines two standard operators, ``+`` and ``&`` to
...
:py:meth:`DataFile.colummn` method. In the third example, the new column data is
...
Removing complete rows of data is achieved using the :py:meth:`DataFile.del_row`
method.::
...
.. warning:
   The :py:class:`Stoner.FileFormats.CSVFile` comma or tab deliminated files discard all metadata 
...
created from scratch, then the :py:meth:`DataFile.save` method will cause a dialog
...
specifies that the deliminator is a tab character.
