...
def test_metadata_basic():
    ...
    fldr6.sort()
    assert repr(fldr6.metadata)=="The DataFolder . has 9 common keys of metadata in 4 Data objects","Representation method of metadata wrong."
    assert len(fldr6.metadata)==9,"Length of common metadata not right."
    assert list(fldr6.metadata.keys())==['Byapp',
                                        'Datatype,Comment',
                                        'Datatype,Time',
                                        'Fileopentime',
                                        'Loaded as',
...
def test_metadata_slice():
    ...
    assert isinstance(fldr6.metadata.slice("Loaded from",values_only=True),list),"metadata.slice not returtning a list with values_only=True."
    assert isinstance(fldr6.metadata.slice("Loaded from",output="Data"),Data),"metadata.slice not returtning Data with outpt='data'."
    for fmt,typ in zip(["dict","list","array","data","frame","smart"],
        ...
        for ix,t in enumerate(typ):
            assert isinstance(ret,t),"Return from slice metadata for output={} and dimension {} had type {} and not {}".format(fmt,ix,type(ret),t)
            try:
                ret=ret[0]
            except (KeyError):
    ...
    for k,typ in zip(['Info.Sample_Holder',('Info.Sample_Holder',"Datatype,Comment")],[(np.ndarray,np.str),(np.ndarray,np.ndarray)]):
        ...
        for ix,t in enumerate(typ):
            assert isinstance(ret,t),"Indexing metadata for key={} and dimension {} had type {} and not {}".format(k,ix,type(ret),t)
            try:
                ret=ret[0]
            except (KeyError):
                ret=ret[list(ret.keys())[0]]
