...
def test_setas_basics():
    ...
    assert selfd4.setas._unique_headers==['Voltage', 'Current', '$\\rho$ ($\\Omega$m)', 'Resistance', 'T (K)', 'Column', 6, 7],"Unique Headers failed in setas."
    selfd4.setas("2.y.x3.")
    s=selfd4.setas.clone
    selfd4.setas.clear()
    selfd4.setas(s)
    ...
    assert all(selfd4.setas.set==np.array([False,False,True,False,True,False,False,False])),"setas.set attribute not working."
    assert all(selfd4.setas.not_set==np.array([True,True,False,True,False,True,True,True])),"setas.set attribute not working."
    assert "x" in selfd4.setas,"set.__contains__ failed"
    del selfd4.setas["x"]
    assert not "x" in selfd4.setas,"setas del item by column type failed."
    del selfd4.setas["rho"]
    assert not "y" in selfd4.setas,"setas del by column named failed"
...
def test_setas_dict_interface():
    ...
    assert selfd4.setas.pop("z",False)==False,"Pop with non existent key and default value failed"
...
def test_setas_metadata():
    ...
    assert d2.setas.to_dict()=={'x': 'Temperature', 'y': 'Resistance', 'e': 'Column 2'},f"__iadd__ failure {repr(d2.setas.to_dict())}"
    ...
    assert selfd2.metadata.type(["User","Timestamp"])==['String', 'Timestamp'],f"Metadata reading error {selfd2.metadata}"
