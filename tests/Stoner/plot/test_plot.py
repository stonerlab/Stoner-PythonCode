...
def test_set_no_figs():
    ...
    e=selfd.clone
    ret=e.plot()
    assert ret is None,"Output of Data.plot() was not None when no_figs is True  and showfig is not set({})".format(type(ret))
    Options.no_figs=False
    e.showfig=False
    ret=e.plot()
    assert isinstance(ret,Data),"Return value of Data.plot() was not self when Data.showfig=False ({})".format(type(ret))
    e.showfig=True
    ret=e.plot()
    assert isinstance(ret,Figure),"Return value of Data.plot() was not Figure when Data.showfig=False({})".format(type(ret))
    e.showfig=None
    ret=e.plot()
