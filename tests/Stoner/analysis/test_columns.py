...
class ColumnOps_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        z=x+4
        dz=np.abs(z/100)
        self.data=Data(np.column_stack((x,y,dy,z,dz)),column_headers=["Tine","Signal 1","d1","Signal 2","d2"])

    def test_add(self):
        self.data.add(1,3,header="Add")
        self.assertTrue(np.all(self.data//"Add" == (self.data//"Signal 1" + self.data//"Signal 2")),"Failed to add column correctly")
    ...
    def test_subtract(self):
        ...
        self.assertTrue(np.all(self.data//"Subtract" == (self.data//"Signal 1" - self.data//"Signal 2")),"Failed to add column correctly")
    ...
    def test_multiply(self):
        ...
        self.assertTrue(np.all((self.data//"Multiply") == ((self.data//"Signal 1") * (self.data//"Signal 2"))),"Failed to add column correctly")
    ...
    def test_stats(self):
        ...
        self.assertTrue(np.allclose(self.data.mean(1,sigma=2),(0.048990998729652346, 0.031144823004794875)),"Channel mean with sigma failed")
