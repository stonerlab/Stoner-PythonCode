...
def deleteCorruptLines(data):
    ...
    for line in data:
        ...
        if string.find(line,'---')!=-1:
            data.pop(data.index(line))
            delCount+=1
    if delCount>6: raw_input('I\'ve detected a lot of bad data in your file you should check it')
    return data
...
def makeTruem(Data):
    ...
    VSMcalibration=Data.data[5,Data.find_col('m (emu)')]/Data.data[5,Data.find_col('X (V)')] #taken from row 5 at random
    for i in range(len(Data.data[:,'0'])):
        Data.data[i,Data.find_col('m (emu)')]=VSMcalibration*Data.data[i,Data.find_col('R (V)')]
    return Data
...
def editData(Data,operations):
    ...
    if 0 in operations: return(Data)
    N=int(raw_input("Input the number of saturated data points on each arm:   "))
    if 1 in operations: Data=makeTruem(Data)
    if 2 in operations: Data.data=driftEliminator(Data.data,N)
    if 3 in operations: Data.data=diamagBackgroundRem(Data.data,N)
    if 4 in operations: Data.data=shift(Data.data,N)
    if 5 in operations: Data=invert(Data)
    return(Data)
...
while(True):
    ...
    fCounter+=1;fCounter%(len(filenames)-1)  #Move counter onto next file
    fr=open(path,'r')  
    data=fr.readlines()  #Get the file into an array
    fr.close()
    pathsplit=splitFileName(path)
    fw=open('EditedFiles/'+pathsplit[0]+"_edit.txt",'w') #create a new file for the edited data
    data=deleteCorruptLines(data)
    fw.writelines(data)    #put the uncorrupted lines into the file so that Stoner can open it
    fw.close()
    while(True):            #open the file
        try: Data=Stoner.VSMFile('EditedFiles/'+pathsplit[0]+'_edit.txt'); break
        except ValueError: 
    ...
    while(True):
        ...
        for i in range(len(strOp.strip())): operations.append(int(strOp[i]))
        t=Data.clone   #edit a copied array.
        t=editData(t,operations)
        plotmH(t)
        whatNext=raw_input('Press enter to save changes, r to restart or q to quit the program:  ')
        if(whatNext=='r'):continue
        elif(whatNext=='q'):break
        else:
            Data=t
            Data.save('EditedFiles/'+pathsplit[0]+'_edit.txt')  #overwrite the file created earlier
            break
    if(whatNext=='q' or raw_input('Press enter to do file {} or q to quit:'.format(filenames[fCounter]))=='q'):  break
plot.close()
plot.ioff()
